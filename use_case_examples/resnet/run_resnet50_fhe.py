...
from resnet import ResNet50_Weights, resnet50_custom
...
def load_model():
    ...
    return resnet50_custom(weights=ResNet50_Weights.IMAGENET1K_V2)
...
def evaluate_model(model, processor):
    ...
    print(f"Accuracy of the ResNet50 model on the images: {accuracy:.2f}%")
    print(f"Top-5 Accuracy of the ResNet50 model on the images: {topk_accuracy:.2f}%")
...
def export_statistics(q_module):
    open("resnet50.graph", "w").write(q_module.fhe_circuit.graph.format(show_locations=True))
    open("resnet50.mlir", "w").write(q_module.fhe_circuit.mlir)
    ...
    with open("resnet50_statistics.json", "w") as f:
...
def evaluate_model_cml(q_module, processor, fhe):
    ...
    topk_accuracy = processor.accuracy_top5(outputs, all_labels)
    if fhe == "simulate":
        print("FHE simulation Accuracy of the FHEResNet50 on the images: " f"{accuracy:.2f}%")
        print(
            "FHE simulation Top-5 Accuracy of the FHEResNet50 on the images: "
            f"{topk_accuracy:.2f}%"
        )
    else:
        print("Quantized Model Accuracy of the FHEResNet50 on the images: " f"{accuracy:.2f}%")
        print(
            "Quantized Model Top-5 Accuracy of the FHEResNet50 on the images: "
            f"{topk_accuracy:.2f}%"
        )
...
def run_experiment(resnet50_model, calib_images, processor, fhe_mode="disable"):
    ...
    for n_bits in n_bits_range:
        for rounding_threshold_bits in rounding_threshold_bits_range:
            ...
            q_module = compile_model(
                resnet50_model, calib_images, n_bits, rounding_threshold_bits, fhe_mode
            )
    ...
    np.save("resnet50_accuracy_matrix.npy", accuracy_matrix)
    ...
    ax.set_title(f"ResNet50 Accuracy of FHE ({fhe_mode})")
    ...
    plt.tight_layout()
    plt.savefig("resnet50_accuracy_matrix.png", dpi=300)
    print("Plot saved as resnet50_accuracy_matrix.png")


def main():
    parser = argparse.ArgumentParser(description="Run ResNet50 model with FHE execution.")
    ...
    evaluate_model(resnet50_model, processor)

    if args.run_experiment:
        # Get the test images and labels
        run_experiment(resnet50_model, calib_images, processor)
    else:
        q_module = compile_model(
            resnet50_model,
            calib_images,
